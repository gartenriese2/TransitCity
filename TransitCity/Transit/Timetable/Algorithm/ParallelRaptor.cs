...
namespace Transit.Timetable.Algorithm
{
    ...
    public class ParallelRaptor<TPos> : ParallelRaptorBase<TPos> where TPos : IPosition
    {
        ...
        public ParallelRaptor(ITimetableManager<TPos, Entry<TPos>> manager, float walkingSpeed, TimeSpan maxWalkingTime, IReadOnlyCollection<TransferStation<TPos>> transferStations)
        ...
        private Dictionary<Station<TPos>, WeekTimePoint> CheckRoute(Entry<TPos> entry, Atomic<AtomicWeekTimePoint> earliestKnownTargetArrivalTime, ConcurrentBag<Connection<TPos>> earliestKnownConnections, TPos targetPos)
        {
